#!/usr/bin/env python3
"""
Generate Dashboard Studio executive_governance.json (default app landing page).
...
OUT = ROOT / "splunk_app" / "splunk_compliance_app" / "default" / "data" / "ui" / "views" / "executive_governance.json"
...
QUERIES = {
    "ds_readiness": (
        ...
        "| table readiness_score governance_status techniques_observed total_techniques"
    ),
    "ds_regulatory_gap": (
        ...
        "| table regulatory_gap governance_status control_efficacy high_severity_events blocked_events"
    ),
    "ds_portfolio_risk": (
...
def build() -> dict:
    data_sources = {k: _ds(k, q) for k, q in QUERIES.items()}

    visualizations = {
        "viz_header": {
            "type": "splunk.markdown",
            "title": "Executive AI Governance — AgentWatch Range",
            "options": {
                "markdown": (
                    "### Executive AI Governance — AgentWatch Range\n\n"
                    "Single-screen posture for **CISO**, **risk**, and **audit** audiences. "
                    "Metrics derive from live OTel telemetry (`otel:agentic:json`), agent registry snapshots "
                    "(`acme:agentic:registry:json`), and framework lookups.\n\n"
                    "**Baseline filter:** attack panels exclude benign traffic with `NOT testbed_mode=BASELINE_TRAFFIC`.\n\n"
                    "**Drill-down:** "
                    "[NIST AI RMF Scoring](/app/acme_genai_compliance/nist_rmf_compliance) · "
                    "[Technique Coverage Matrix](/app/acme_genai_compliance/technique_coverage_matrix) · "
                    "[Threat Hunting](/app/acme_genai_compliance/threat_hunting) · "
                    "[Cross-App Normalization](/app/acme_genai_compliance/cross_app_normalization) · "
                    "[Exercise Runner](/app/acme_genai_compliance/exercise_runner)"
                )
            },
        },
        "viz_readiness": {
            "type": "splunk.singlevalue",
            "title": "Framework Readiness Score (% techniques with telemetry)",
        ...
        "viz_regulatory_gap": {
            "type": "splunk.singlevalue",
            "title": "Regulatory Gap Metric (% high-severity events without control block)",
            "dataSources": {"primary": "ds_regulatory_gap"},
            "options": {"field": "regulatory_gap", "unit": "%", "unitPosition": "after"},
        },
        "viz_regulatory_detail": {
            "type": "splunk.table",
            "title": "Control efficacy on Critical/High severity",
            "dataSources": {"primary": "ds_regulatory_gap"},
            "options": {"count": 5},
        },
        "viz_portfolio": {
            "type": "splunk.table",
            "title": "AI Portfolio Risk Matrix (registry × attack volume)",
            "dataSources": {"primary": "ds_portfolio_risk"},
            "options": {"count": 15, "wrap": True},
        },
        "viz_agent_counts": {
            "type": "splunk.table",
            "title": "Agent inventory — running vs shadow (latest snapshot in window)",
            "dataSources": {"primary": "ds_agent_counts"},
            "options": {"count": 5},
        },
        "viz_agent_registry": {
            "type": "splunk.table",
            "title": "Registered agents (deduped latest per agent_id)",
            "dataSources": {"primary": "ds_agent_registry"},
            "options": {"count": 10, "wrap": True},
        },
        "viz_guardrail_chart": {
            "type": "viz.line",
            "title": "Guardrail blocks over time (by technique)",
            "dataSources": {"primary": "ds_guardrail_timechart"},
        },
        "viz_eu_ai_act": {
            "type": "splunk.table",
            "title": "EU AI Act risk tier — crosswalk inventory",
            "dataSources": {"primary": "ds_eu_ai_act"},
            "options": {"count": 10},
        },
        "viz_hitl": {
            "type": "splunk.table",
            "title": "Human-in-the-loop governance events",
    ...
    return {
        ...
        "description": (
            "CISO and auditor front door — framework readiness, regulatory control gap, portfolio risk, "
            "agent inventory (running vs shadow), and guardrail efficacy. Dashboard Studio view."
        ),
        "inputs": {
            "input_time": {
        ...
        "layout": {
            ...
            "tabs": {
                "items": [{"layoutId": "layout_main", "label": "Executive AI Governance"}],
                "options": {"barPosition": "top", "showTabBar": False},
            },
            "layoutDefinitions": {
                "layout_main": {
                    "type": "absolute",
                    "options": {"display": "auto-scale", "width": W, "height": 2100},
                    "structure": structure,
                }
            },
        },
...
def main() -> None:
    ...
    assert dashboard["visualizations"]["viz_header"]["type"] == "splunk.markdown"
    assert "layoutDefinitions" in dashboard["layout"]
    print(f"Wrote {OUT} (Dashboard Studio, splunk.markdown header, tabs schema)")
