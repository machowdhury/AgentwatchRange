...
Dashboard Studio view: **Executive AI Governance** (`executive_governance.xml`, Splunk `version="2"` with JSON in `<definition>`) — the default landing page for CISO, risk, and audit audiences in the **AgentWatch Range — GenAI Compliance** Splunk app.
